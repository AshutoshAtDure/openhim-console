...
app.run(function ($templateCache) {
  ...
  $templateCache.put('views/partials/rowData.html', require('~/views/partials/rowData.html'))
})
...
app.run(function ($rootScope, $location, $anchorScroll, Api) {
  $rootScope.$on('$routeChangeStart', function () {
    ...
    if (curRoute !== '/' && curRoute.indexOf('/login') === -1 && curRoute !== '/logout' && curRoute.indexOf('/forgot-password') === -1 && curRoute.indexOf('/set-password') === -1 && curRoute.indexOf('/registration') === -1) {
    ...
    if (curRoute && !consoleSession && curRoute.indexOf('login') !== 1 && curRoute.indexOf('set-password') !== 1 && curRoute.indexOf('forgot-password') !== 1 && curRoute.indexOf('/registration') === -1) {
      $location.path('/login')
    }

    Api.Me.get(function (authDetails) {
      localStorage.setItem('userDetails', JSON.stringify(authDetails.user))
      if (authDetails.user) {
      ...
      if (consoleSession) {
        ...
        if (page.indexOf('set-password') !== 1 && page.indexOf('forgot-password') !== 1 && page.indexOf('login') !== 1 && curRoute.indexOf('/registration') === -1) {
...
app.config(function ($routeProvider) {
  $routeProvider
    ...
    .when('/newFile', {
      template: require('~/views/newFile'),
      controller: controllers.AboutCtrl
    })
    .when('/orgLanding', {
      template: require('~/views/orgLandingPage.html'),
    })
    .when('/registration', {
      template: require('../views/registration.html'),
      controller: 'RegistrationCtrl'
    })
    .when('/configuration', {
      template: require('~/views/configuration'),
      controller: 'configurationCtrl'
    })
    .when('/pendingRequests', {
      template: require('~/views/pendingRequests'),
      controller: 'PendingUsersCtrl'
    })
    .when('/pendingUserRequest/:userId/:state', {
      template: require('../views/pendingUsersView.html'),
      controller: 'PendingUserViewCtrl'
    })
    .when('/manageConfig/:registrationId', {
      template: require('~/views/manageConfig.html'),
      controller: 'ManageConfigCtrl'
    })
    .when('/batchDataTransfer', {
      template: require('../views/batchDataTransfer.html'),
      controller: 'BatchDataTransferCtrl'
    })
    .when('/orgProfile', {
      template: require('../views/orgProfile.html'),
      controller: 'OrgProfileCtrl'
    })
    .when('/demo', {
      template: require('~/views/demo'),
      // controller: controllers.AboutCtrl
    })
    .when('/audit', {
      template: require('~/views/audit.html'),
